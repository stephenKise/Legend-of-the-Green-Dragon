<?php
	...
	if( $cityid > 0 )
	{
		...
		{
			$row['mods'] = is_serialized($row['cityblockmods']) ? @unserialize($row['cityblockmods']) : '';
			$row['navs'] = is_serialized($row['cityblocknavs']) ? @unserialize($row['cityblocknavs']) : '';
			$row['vill'] = is_serialized($row['citytext']) ? @unserialize($row['citytext']) : '';
			$row['stab'] = is_serialized($row['stabletext']) ? @unserialize($row['stabletext']) : '';
			$row['arm'] = is_serialized($row['armortext']) ? @unserialize($row['armortext']) : '';
			$row['weap'] = is_serialized($row['weaponstext']) ? @unserialize($row['weaponstext']) : '';
			$row['merc'] = is_serialized($row['mercenarycamptext']) ? @unserialize($row['mercenarycamptext']) : '';
	...
	$form = array(
		'City Details,title',
			...
			'citytravel'=>'Travel:,enum,0,Safe,1,Dangerous,2,Off',
