...
function page_header()
{
    ...
    $nopopups = array_merge($nopopups, $tempNo);
    unset($tempNo);
    ...
    // Run more checks on this.
    $runheaders[$script] = $runheaders[$script] ?: false;
...
{
    $s = explode('x', $size);
    $regex = preg_replace("([^[:alnum:]])", '', $page);
    return "window.open('$page','$regex','scrollbars=yes, resizable=yes, width={$s[0]}, height={$s[1]}').focus()";
}
...
<input type="image" src="images/paypal2.gif" border="0" name="submit" alt="Donate!">
    ...
    // Replace core JavaScript functions.
    $header = str_replace("{script}", $script, $header);
    $javaScript = modulehook("javascript");
    if (!empty($javaScript)) {
...
function addcharstat($label, $value = false)
{
    global $charstat_info, $last_charstat_label;
    if ($value === false) {
        $charstat_info[$label] = $charstat_info[$label] ?: [];
...
function maillink()
{
    global $session;
    $mail = db_prefix('mail');
    $sql = "SELECT SUM(if(seen=1,1,0)) AS seencount, SUM(if(seen=0,1,0)) AS notseen
        FROM $mail WHERE msgto='{$session['user']['acctid']}'";
    $result = db_query_cached($sql, "mail-{$session['user']['acctid']}", 604800);
    $row = db_fetch_assoc($result);
    db_free_result($result);
    $pop = popup('mail.php');
