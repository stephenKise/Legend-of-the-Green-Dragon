...
rawoutput(
        ...
        </form>", htmlent($row['lastip'] ?: ''), htmlent($row['uniqueid'] ?: ''), httpget('reason')
        )
);
output(
...
if ($row['name'] != '') {
    ...
    output(
            "`\$Banning`@ by `QID `^(%s)`@ will effect the following accounts:`0`n", $id
    );
    $sql = db_query(
            "SELECT name, login
    ...
    while ($row = db_fetch_assoc($sql)) {
        output_notl(
                "`%%s `^(%s)`0`n", $row['name'], $row['login']
        );
    }
    output_notl("`n");
    output(
            "`\$Banning `@by `Qsimilar IP `^(%s)`@ will effect the following accounts:`0`n", $ip
    );
    ...
    while ($row = db_fetch_assoc($sql)) {
        ...
        output(
                "(%s) [%s] `%%s`0 - %s hits, last: %s`n", $row['lastip'], $row['uniqueid'], $row['name'], $row['gentimecount'], reltime($lastOn)
        );
    }
}
?>