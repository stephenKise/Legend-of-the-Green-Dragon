...
function datacache(string $name, int $duration)
{
    global $datacache, $queriesFromCache, $DB_USEDATACACHE;
    if ($DB_USEDATACACHE == 0) {
        return false;
    }
...
{
    return datacacheWipeContaining($name);
}

function datacacheWipeContaining(string $name): bool
{
