...
if (!isset($session['fromversion']) || $session['fromversion'] == '') {
	...
	$upgradeChecked = $session['dbinfo']['upgrade'] ? 'checked' : '';
	rawoutput("<input type='radio' value='upgrade' name='type' $upgradeChecked>");
	...
	rawoutput("</select>");
	rawoutput("<br><input type='radio' value='install' name='type' $upgradeChecked>");
