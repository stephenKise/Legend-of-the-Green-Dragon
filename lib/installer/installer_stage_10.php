...
$result = db_query(
	"SELECT login, password FROM $accountsPrefix WHERE superuser & $megaUser"
);
...
if (db_num_rows($result) == 0) {
	if ($name > '') {
		...
		elseif (strlen(httppost('pass1')) < 6) {
			output("
			`\$Whoa, that's a short password, you really should make it longer.`2`n
			");
			$showform = true;
		}
		else {
			...
			db_query(
				"INSERT INTO $accountsPrefix
				(login, password, superuser, name, ctitle, regdate, laston)
				VALUES
				('$name', '$pass', $su, '`%Admin `&$name`0','`%Admin', NOW(), NOW())"
			);
